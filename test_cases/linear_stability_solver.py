...
def gep(ny, nz, k, mu, case, dim): 
    ...
    #evals, evecs = eigs(sA, k=5, M=sB, which='LI', v0 = np.ones(sA.shape[0]), sigma=-0.03+0.07*1j)
...
ny, nz, k, mu, case, dim = int(sys.argv[1]), int(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4]), str(sys.argv[5]), str(sys.argv[6])
    
evals, evecs = gep(ny, nz, k, mu, case, dim)

cs = evals[np.argmax(evals.imag)]; print(cs)
