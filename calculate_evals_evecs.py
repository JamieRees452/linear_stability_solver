...
ny, nz, k, case, init_guess, integration, stability = int(sys.argv[1]), int(sys.argv[2]), float(sys.argv[3]), str(sys.argv[4]), complex(sys.argv[5]), str(sys.argv[6]), str(sys.argv[7])
...
if os.path.exists(fname[0]) and os.path.exists(fname[1]): # If the files (fname) exist then we must decide whether to overwrite existing saved eigenvalues or not
    ...
    if user_input == 'y':
        ...
        evals, evecs = sparse_solver.gep(ny, nz, k, case, integration, stability, init_guess, values=1, tol_input=0, iter_input=1e6)
        end_time = time.time()
