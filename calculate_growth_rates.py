...
if case == 'NEMO':
    fname = f'/home/rees/lsa/growth_rate/growth_{case}_{integration}_{stability}_{assume}_{ny:02}_{nz:02}_{guess_re}_{guess_im}.txt'
else:
...
if os.path.exists(fname):
    ...
    if user_input == 'y':
        ...
        for count, k in enumerate(tqdm(k_wavenum, position=0, leave=True)):
            cs[:, count] = sparse_solver.gep(ny, nz, k, case, integration, stability, assume, init_guess, values, tol_input=1e-6, iter_input=1e5)[0]
